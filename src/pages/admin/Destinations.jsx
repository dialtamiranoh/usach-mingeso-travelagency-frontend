...
const Destinations = () => {
    ...
    const fetchDestinations = () => {
        ...
        DestinationService.getAll()
            .then(response => { setDestinations(response.data); setLoading(false) })
            .catch(() => { showAlert('danger', 'Error al cargar los destinos'); setLoading(false) })
    }

    const fetchStatuses = () => {
        StatusService.getByEntityType('DESTINATION')
    ...
    const handleCreate = () => {
        ...
        DestinationService.create(formData)
            .then(() => {
                ...
                fetchDestinations()
                showAlert('success', 'Destino creado correctamente')
            })
            .catch(() => showAlert('danger', 'Error al crear el destino'))
    }

    const handleEditClick = (destination) => {
        setSelectedDestination(destination)
        setFormData({ name: destination.name, description: destination.description || '', status: { id: destination.status?.id || '' } })
    ...
    const handleEdit = () => {
        ...
        DestinationService.update(selectedDestination.id, formData)
            .then(() => {
                setShowEditModal(false)
                setSelectedDestination(null)
                fetchDestinations()
                showAlert('success', 'Destino editado correctamente')
            })
            .catch(() => showAlert('danger', 'Error al editar el destino'))
    }
    ...
    const handleDelete = () => {
        DestinationService.remove(selectedDestination.id)
            .then(() => {
                setShowDeleteModal(false)
                setSelectedDestination(null)
                fetchDestinations()
                showAlert('success', 'Destino eliminado correctamente')
            })
            .catch(() => showAlert('danger', 'Error al eliminar el destino'))
    }
            ...
            <table className="table table-striped table-hover">
                ...
                <tbody>
                    {destinations.length === 0 ? (
                        <tr><td colSpan={6} className="text-center">No hay destinos registrados</td></tr>
                    ) : (
                        destinations.map(destination => (
                            <tr key={destination.id}>
                                ...
                                <td>{destination.status?.name}</td>
                                <td>{new Date(destination.createdAt).toLocaleDateString()}</td>
                                <td>
                                    <button className="btn btn-sm btn-warning me-2" onClick={() => handleEditClick(destination)}><FaEdit /> Editar</button>
                                    <button className="btn btn-sm btn-danger" onClick={() => handleDeleteClick(destination)}><FaTrash /> Eliminar</button>
                                </td>
                            </tr>
                        ))
                    )}
                    ...
                    <div className="modal-dialog">
                        <div className="modal-content">
                            <div className="modal-header bg-primary text-white">
                                <h5 className="modal-title">Nuevo Destino</h5>
                ...
                <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ¿Estás seguro de eliminar el destino <strong>{selectedDestination?.name}</strong>?
                            </div>
                            <div className="modal-footer">
